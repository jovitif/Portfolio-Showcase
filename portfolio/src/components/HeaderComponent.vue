<template>
  <v-app-bar app color="white" flat>
    <v-container>
      <v-row align="center" justify="space-between">
        <!-- Logo -->
        <v-col cols="auto">
          <a class="d-flex align-center" href="index.html">
            <h1 class="mb-0" style="color: #0a66c2; font-size: 23px;">JOVITIF</h1>
          </a>
        </v-col>

        <!-- Menu Toggle (para telas pequenas) -->
        <v-col cols="auto" class="d-md-none">
          <v-btn icon @click="toggleMenu">
            <v-icon>{{ isMenuOpen ? 'mdi-menu-open' : 'mdi-menu' }}</v-icon>
          </v-btn>
        </v-col>

        <!-- Navegação para telas grandes -->
        <v-col cols="auto" class="d-none d-md-flex">
          <v-toolbar-items>
            <v-btn text v-for="item in menuItems" :key="item.label" :href="item.link" class="text--grey">
              {{ item.label }}
            </v-btn>
          </v-toolbar-items>
        </v-col>
      </v-row>

      <!-- Menu de navegação para telas pequenas -->
      <v-slide-y-transition>
        <v-row v-if="isMenuOpen" class="d-flex d-md-none" justify="center">
          <v-col cols="12">
            <v-list dense>
              <v-list-item v-for="item in menuItems" :key="item.label">
                <v-list-item-content>
                  <v-btn text :href="item.link" class="text--grey">{{ item.label }}</v-btn>
                </v-list-item-content>
              </v-list-item>
            </v-list>
          </v-col>
        </v-row>
      </v-slide-y-transition>
    </v-container>
  </v-app-bar>
</template>

<script>
export default {
  name: 'HeaderComponent',
  data() {
    return {
      isMenuOpen: false,
      menuItems: [
        { label: 'About', link: '#About' },
        { label: 'Skills', link: '#Skills' },
        { label: 'Experience', link: '#Experience' },
        { label: 'Projects', link: '#Projects' },
        { label: 'Certifications', link: '#Certifications' },
        { label: 'Contact', link: '#Contact' }
      ]
    };
  },
  methods: {
    toggleMenu() {
      this.isMenuOpen = !this.isMenuOpen;
    }
  }
};
</script>

<style scoped>
.v-btn {
  color: #666666;
}
.v-btn:hover {
  color: black;
}
</style>
